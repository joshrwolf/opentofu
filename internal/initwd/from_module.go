...
func DirFromModule(ctx context.Context, loader *configload.Loader, rootDir, modulesDir, sourceAddrStr string, reg *registry.Client, remoteFetcher *getmodules.PackageFetcher, hooks ModuleInstallHooks) tfdiags.Diagnostics {
	...
	fakeRootModule := &configs.Module{
		ModuleCalls: map[string]*configs.ModuleCall{
			initFromModuleRootCallName: {
				...
				DeclRange:  rng,
				Variables: func(_ context.Context, v *configs.Variable, _ configs.EvalOverlay) (cty.Value, hcl.Diagnostics) {
	...
	for _, recordKey := range recordKeys {
		...
		if record.Key == initFromModuleRootCallName {
			...
			mod, _ := loader.Parser().LoadConfigDir(rootDir, configs.NewStaticModuleCall(addrs.RootModule, hcl.Range{}, func(_ context.Context, v *configs.Variable, _ configs.EvalOverlay) (cty.Value, hcl.Diagnostics) { // ignore diagnostics since we're just doing value-add here anyway
				if v.Default != cty.NilVal {
					return v.Default, nil
				}
				return cty.DynamicVal, nil
