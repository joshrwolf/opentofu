...
func (m *Meta) rootModuleCall(ctx context.Context, rootDir string) (configs.StaticModuleCall, tfdiags.Diagnostics) {
	...
	call := configs.NewStaticModuleCall(addrs.RootModule, hcl.Range{}, func(_ context.Context, variable *configs.Variable, _ configs.EvalOverlay) (cty.Value, hcl.Diagnostics) {
		name := variable.Name
		v, ok := variables[name]
		if !ok {
